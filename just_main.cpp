		...
		if (_kbhit()) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (choise == 0) {
					...
					SetPos(25, 20); cout << "Medium    ";
				}
				else if (choise == 2) {
					SetPos(25, 20); cout << "Hard      ";
				}
			}
...
int main() {	
	...
	while (1) {
		...
		if (key == 72) {
			...
			if (choise == 0) { system("cls"); int k = select_main_map(); 
			if (k == 1) {
				m.setMode(1); start(1000, 1000, 5, 4);
				m.download_Bin("front", 0);
				m.start_game(1); system("cls");
			}
			else if (k == 2) {
				m.setMode(2); start(1000, 1000, 5, 4);
				m.download_Bin("puzzle", 0);
				m.start_game(1); system("cls");
			}
			else if (k == 3) {
				m.setMode(3); start(1000, 1000, 5, 4);
